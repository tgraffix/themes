...
if( ! class_exists( 'WOODMART_HB_Header' ) ) {
	class WOODMART_HB_Header {
		...
		public function set_structure( $structure = false ) {
			if( ! $structure ) $structure = $this->get_default_structure();
		...
		public function get_default_structure() {
			...
			$default_structure = woodmart_get_config('header-builder-structure');

			$structure = $this->fill_sceleton_with_elements( $sceleton, $default_structure );
