...
if ( ! class_exists( 'ReduxFramework_woodmart_multi_fonts' ) && class_exists( 'ReduxFramework' ) ) {
    ...
    class ReduxFramework_woodmart_multi_fonts extends ReduxFramework {
        ...
        public function render() {
            ...
            //Add new btn
            echo '<a href="javascript:void(0);" class="button button-alternative woodmart-multi-fonts-add" data-id="' . esc_attr( $this->field['id'] ) . '-ul"  data-name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '" data-order="' . esc_attr( $this->order ) . '">'. esc_html__( 'Add More', 'woodmart' ) .'</a>';
        }
        ...
        public function fields( $type, $multi_value = false ) {
            ...
            $font_weight_values = array(
                ...
                'opt' => array( 100, 200, 300, 400, 500, 600, 700, 800, 900 ),
            );
                    ...
                    echo '<div class="miltifont-field">';
                        ...
                        echo '<select name="' . esc_attr( $font_weight_values['name'] ) . '">';
                            foreach ( $font_weight_values['opt'] as $key => $opt_val ) {
                                ...
                                echo '<option value="' . esc_attr( $opt_val ) . '" ' . esc_attr( $selected ) . '>' . esc_attr( $opt_val ) . '</option>';
                            }
                        echo '</select>';
                    echo '</div>';
        ...
        public function mediaFields( $title, $value, $name, $mime, $id ) {
            ...
            echo '<div class="miltifont-field">';
                ...
                echo '<fieldset class="redux-field-container redux-field redux-container-media redux-field-init multifonts" id="' . esc_attr( uniqid() ) . '-media">';
